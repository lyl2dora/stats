...
class Helper: NSObject, NSXPCListenerDelegate, HelperProtocol {
    ...
    private let smcQueue = DispatchQueue(label: "zone.lyl.stats.SMC.Helper.smcQueue")
    ...
    override init() {
        self.listener = NSXPCListener(machServiceName: "zone.lyl.stats.SMC.Helper")
        super.init()
    ...
    private func uninstallHelper() {
        ...
        process.arguments = ["unload", "/Library/LaunchDaemons/zone.lyl.stats.SMC.Helper.plist"]
        do {
            try process.run()
        ...
        do {
            try FileManager.default.removeItem(at: URL(fileURLWithPath: "/Library/LaunchDaemons/zone.lyl.stats.SMC.Helper.plist"))
        } catch let err {
        ...
        do {
            try FileManager.default.removeItem(at: URL(fileURLWithPath: "/Library/PrivilegedHelperTools/zone.lyl.stats.SMC.Helper"))
        } catch let err {
...
extension Helper {
    ...
    func uninstall() {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/Library/PrivilegedHelperTools/zone.lyl.stats.SMC.Helper")
        process.qualityOfService = QualityOfService.userInitiated
